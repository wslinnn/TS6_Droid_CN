...
### v2.4.0-Han（2026-09-01）

**界面与体验**
- 连接服务器后顶栏新增设置入口：通话中可直接调整外观（悬浮窗/动漫背景/自定义壁纸）、音频（增益/降噪/麦克风模式/VAD 阈值）与聊天设置（自动重连/链接缩略图/自动加载图片），无需断开连接；身份与语言等仅连接前有意义的设置仍保留在连接页

**安装包**
- 正式包启用 R8 代码压缩与资源收缩，并仅保留 arm64-v8a 架构：下载体积从约 70MB 降至约 15MB

**修复**
- 修复服务器信息卡「频道数 / 在线人数 / 运行时长」显示为 0 的问题
- 修复使用域名作为服务器地址时连接闪退的问题

### v2.3.1-Han（2026-09-01）

**界面与体验**
- 应用内「检查更新」的更新说明改为 Markdown 渲染：标题、加粗、列表、分隔线与链接按排版展示（引入 multiplatform-markdown-renderer 组件）
- 关于页移除「贡献者」板块，页面精简为版权说明、仓库链接与免责声明
